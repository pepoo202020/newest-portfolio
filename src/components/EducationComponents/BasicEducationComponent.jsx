...
const BasicEducation = styled(motion.div)`
  display: flex;
  & > * {
    flex: 1;
  }
...
const BasicEducationImage = styled.div`
  display: flex;
  align-items: center;
...
const BasicEducationContent = styled.div`
  text-align: center;
  ...
  justify-content: center;
`;
const BasicEducationHeader = styled(motion.h1)`
  font-size: 56px;
  font-weight: 400;
  line-height: 1.1;
  text-align: center;
...
const BasicEducationSubHeader = styled(motion.h3)`
  font-size: 30px;
  text-align: center;
  @media (max-width: 768px) {
    font-size: 20px;
...
const CertificationItemImage = styled.img`
  height: 50px;
  margin: 30px 0;
`;

const BasicEducationComponent = () => {
  const theme = useSelector((state) => state.themes.selectedTheme);
  const { t } = useTranslation();

  const certifications = [
    {
      siteName: t("certification_name_eng"),
      icon: "https://www.aun.edu.eg/engineering/sites/default/files/best%20eng%20logo.png",
      link: "https://www.aun.edu.eg/engineering/ar/alsfht-alryysyt",
    },
    {
      siteName: t("certification_name_ud"),
      icon: "https://cdn.iconscout.com/icon/free/png-256/free-udacity-3772434-3151560.png",
      link: "https://www.udacity.com/",
    },
    {
      siteName: t("certification_name_eg"),
      icon: "https://benaeducation.com/wp-content/uploads/2021/03/Egypt_fwd_logo-1.png",
      link: "https://egfwd.com/",
    },
  ];
  return (
    <BasicEducation
      ...
      <BasicEducationImage>
        ...
        >
          ...
          <path
            ...
            transform="translate(-202.30284 -162.98099)"
            fill={theme === "light" ? "#1D3557" : "#F1FAEE"}
          />
          <path
            d="M407.19716,520.481l-24,36s8,37,185,36,191-36,191-36l-33-48-101-69Z"
            transform="translate(-202.30284 -162.98099)"
          ...
          <path
            ...
            fill={theme === "light" ? "#1D3557" : "#F1FAEE"}
          />
          <ellipse cx="553.39431" cy="272" rx="29" ry="20" fill="#A8DADC" />
        </svg>
      </BasicEducationImage>
      <BasicEducationContent colortheme={theme}>
        <BasicEducationHeader
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5, duration: 0.5 }}
        >
          {t("basic_education_header")}
        </BasicEducationHeader>
        <BasicEducationSubHeader
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.7, duration: 0.5 }}
        >
          {t("basic_education_sub_header")}
        </BasicEducationSubHeader>
        <CertificationLogoContainer certificationsContent={certifications} />
      </BasicEducationContent>
    </BasicEducation>
  );
};

export default BasicEducationComponent;

const CertificationLogoContainer = ({ certificationsContent }) => {
  return (
    <DivIcons>
      {certificationsContent.map((cer, index) => (
        <OverlayTrigger
          key={index}
          placement="bottom"
          overlay={
            <Tooltip id="tooltip-top">
              <strong>{cer.siteName}</strong>
            </Tooltip>
          }
        >
          <li>
            <a href={cer.link} target="_blank" rel="noopener noreferrer">
              <span data-inline="false">
                <CertificationItemImage src={cer.icon} />
              </span>
            </a>
          </li>
        </OverlayTrigger>
